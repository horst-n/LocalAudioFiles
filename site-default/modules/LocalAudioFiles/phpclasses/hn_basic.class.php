...
require_once( dirname(__FILE__).'/hn_basic_helpers.class.php' );
	...
	{
		 // If it isn't set, scripts run in commandline mode and we set it to an empty string
		$_SERVER['DOCUMENT_ROOT'] = '';
	}



	if(!function_exists('hn_studiologger'))
	{
		function hn_studiologger( $logcategory,$s1='',$s2='',$s3='',$s4='',$s5='',  $DEBUG=false, $VERBOSE=false )
		{
			if(!class_exists('hn_studiolog_sender'))
			{
				if(file_exists('C:/bin_local/PHP_includes/hn_studiolog.class.php'))
				{
					require_once('C:/bin_local/PHP_includes/hn_studiolog.class.php');
				}
				elseif(file_exists(dirname(__FILE__) . '/hn_studiolog.class.php'))
				{
					require_once(dirname(__FILE__) . '/hn_studiolog.class.php');
				}
			}
			if( ! class_exists('hn_studiolog_sender') )
			{
			...
			$sender = md5( $logcategory.$s1.$s2.$s3.strval(time()) );
			$GLOBALS[$sender] = new hn_studiolog_sender( $sender );
			$GLOBALS[$sender]->DebugOutMode = 2;
			$GLOBALS[$sender]->debug = $DEBUG === true ? true : false;
			$GLOBALS[$sender]->verbose = $VERBOSE === true ? true : false;
			return $GLOBALS[$sender]->send_logdata( $logcategory, $s1, $s2, $s3, $s4, $s5 );
		}
	}
