...
// 'file_put_contents' is only available since PHP 5.0.0
if( ! defined('LOCK_EX') )                define('LOCK_EX', 2);
if( ! defined('LOCK_UN') )                define('LOCK_UN', 3);
if( ! defined('FILE_APPEND') )            define('FILE_APPEND', 8);
if( ! defined('FILE_USE_INCLUDE_PATH') )  define('FILE_USE_INCLUDE_PATH', 1);
if( ! function_exists('file_put_contents') )
{
    # $int_flags is any combination of FILE_APPEND (8), LOCK_EX (2), FILE_USE_INCLUDE_PATH (1)
    #
    function file_put_contents( $str_file, $data, $int_flags=0, $zcontext=null )
    {
        $lock = in_array( $int_flags, array(2,3,10,11) ) ? true : false;
        $mode = in_array( $int_flags, array(8,9,10,11) ) ? 'ab' : 'wb';
        if( $lock )
        {
            @touch( $str_file );
...
if( ! function_exists('is_executable') )
{
    ...
    function is_executable($filename)
    {
        ...
        return in_array(strtoupper($ext),$a) || in_array(strtolower($ext),$a) ? true : false;
    }
}
